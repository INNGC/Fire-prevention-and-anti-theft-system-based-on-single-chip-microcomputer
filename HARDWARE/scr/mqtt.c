...
char pingFlag; // ping报文状态       0：正常状态，等待计时时间到，发送Ping报文
			   // ping报文状态       1：Ping报文已发送，当收到 服务器回复报文的后 将1置为0
char connectFlag;	   //同服务器连接状态   0：还没有连接服务器  1：连接上服务器了
char connectPackFlag;  // CONNECT报文状态    1：CONNECT报文成功
char subcribePackFlag; //订阅报文状态       1：订阅报文成功
...
void MQTT_DealPushdata_Qs0(unsigned char *redata)
{
	...
	for (i = 0; i < pack_num; i++) //已经统计到了接收的推送个数，开始for循环，取出每个推送的数据
	{
		do
		{
			temp = data[totle_len + local];//剩余长度=可变报头长度+负载长度
			temp_len += (temp & 127) * multiplier;
			multiplier *= 128;
			local++;
		} while ((temp & 128) != 0);
