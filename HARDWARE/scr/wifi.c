...
unsigned char *p = "AT+CWJAP=\"AUBDT\",\"2963754610\"";
...
char WiFi_JoinAP(int timeout)
{
	...
	while (timeout--)									   //等待超时时间到0
	{
		...
		if (strstr(WiFi_RX_BUF, "OK"))			   //如果接收到WIFI GOT IP表示成功
			break;								   //主动跳出while循环
		u1_printf("%d %s", timeout, WiFi_RX_BUF); //串口输出现在的超时时间
	}
	u1_printf("\r\n%s\r\n", WiFi_RX_BUF);
	u1_printf("\r\n"); //串口输出信息
	if (timeout <= 0)
...
/*返回值：0：正确   其他：错误                     */
/*-------------------------------------------------*/
char WIFi_Is_Connect(int timeout)
{
	WiFi_RxCounter = 0;									   // WiFi接收数据量变量清零
	memset(WiFi_RX_BUF, 0, WiFi_RXBUFF_SIZE);			   //清空WiFi接收缓冲区
	WiFi_printf("AT+CWJAP?");							//获取WIFI连接状态
	while (timeout--)						  //等待超时时间到0
	{
		DelayMs(1000);						  //延时1s
		if (!strstr(WiFi_RX_BUF, "AT+CWJAP?")) //如果串口接受到OK表示成功
			break;							  //跳出while循环
		u1_printf("%d", timeout);			  //串口输出现在的超时时间
	}
	u1_printf("\r\n%s\r\n", WiFi_RX_BUF);
...
char WiFi_Connect_IoTServer(void)
{
	...
	u1_printf("准备连接服务器\r\n"); //串口提示数据
	if (WiFi_Connect_Server(20))	 //连接服务器,100ms超时单位,总计10s超时时间
	{
